$(document).ready(function() {
  ...
  var cities = [""];
  var key = "974d57fc4741c11d5e34e23cd38dcddb";
  ...
  function init(response) {
    ...
    var temp = $("<p>");
    temp.text(
      "Temperature: " + Math.floor(response.main.temp * 1.8 + 32) + " °F"
    );
    var hum = $("<p>");
  ...
  function forecast(response) {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      icon.attr("style", "width:3rem; height:3rem;");
      temp.html(
        "Temperature: <br> " + Math.floor(forecastData[i].main.temp) + " °F"
      );
      var hum = $("<p>");
  ...
  $(document).on("click", "button", function() {
    ...
    var queryURL =
      "https://api.openweathermap.org/data/2.5/weather?" + c + "&appid=" + key;
    $.ajax({
  ...
  $(document).ready(function() {
    ...
    var queryURL =
      "https://api.openweathermap.org/data/2.5/weather?q=chicago&appid=" + key;
    $.ajax({
      url: queryURL, //"London,uk&appid=b6907d289e10d714a6e88b30761fae22",
      method: "GET"
    }).then(function(response) {
    ...
    var forecastURL =
      "https://api.openweathermap.org/data/2.5/forecast?q=Chicago&appid=" +
      key +
      "&units=imperial";
    $.ajax({
      url: forecastURL,
